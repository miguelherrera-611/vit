...
// Reutilizamos el mismo CSS de Create para consistencia
const CSS = `
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .uf-root { min-height:100vh; font-family:'DM Sans',sans-serif; background: radial-gradient(ellipse 70% 55% at 5% 0%,rgba(255,215,175,0.18) 0%,transparent 55%), radial-gradient(ellipse 55% 50% at 95% 100%,rgba(255,195,145,0.13) 0%,transparent 55%), linear-gradient(150deg,#fdf6f0 0%,#fdf3ec 40%,#fef5ef 70%,#fef8f4 100%); }
    .uf-header { background:rgba(253,246,240,0.75); backdrop-filter:blur(28px) saturate(160%); border-bottom:1px solid rgba(200,140,80,0.1); position:sticky; top:0; z-index:40; }
    .uf-header-inner { max-width:900px; margin:0 auto; padding:1.25rem 1.5rem; display:flex; align-items:center; gap:1rem; }
    .uf-back { width:32px; height:32px; border-radius:8px; flex-shrink:0; display:flex; align-items:center; justify-content:center; background:rgba(255,255,255,0.4); border:1px solid rgba(200,140,80,0.15); color:rgba(150,80,20,0.55); text-decoration:none; transition:all 0.14s; }
    ...
    .uf-section { background:rgba(255,255,255,0.48); backdrop-filter:blur(20px) saturate(150%); border:1px solid rgba(200,140,80,0.12); border-radius:16px; padding:1.75rem; box-shadow:0 4px 24px rgba(180,90,20,0.05),inset 0 1px 0 rgba(255,255,255,0.9); animation:fadeUp 0.4s cubic-bezier(0.16,1,0.3,1) both; }
    .uf-step-num { width:26px; height:26px; border-radius:7px; flex-shrink:0; display:flex; align-items:center; justify-content:center; font-size:0.72rem; font-weight:600; background:rgba(185,28,28,0.08); border:1px solid rgba(185,28,28,0.18); color:rgba(185,28,28,0.8); }
    ...
    .uf-label-opt { font-weight:400; text-transform:none; letter-spacing:normal; color:rgba(150,80,20,0.35); margin-left:0.3rem; }
    .uf-input { padding:0.72rem 0.9rem; background:rgba(255,255,255,0.55); border:1px solid rgba(200,140,80,0.18); border-radius:9px; font-size:0.84rem; color:#2d1a08; font-family:'DM Sans',sans-serif; outline:none; transition:all 0.14s; }
    ...
    .uf-role-card { padding:1.25rem; border-radius:12px; cursor:pointer; text-align:left; border:1.5px solid rgba(200,140,80,0.15); background:rgba(255,255,255,0.35); transition:all 0.16s; font-family:'DM Sans',sans-serif; }
    ...
    .uf-info-box { padding:1rem 1.1rem; border-radius:10px; background:rgba(185,28,28,0.04); border:1px solid rgba(185,28,28,0.12); display:flex; align-items:flex-start; gap:0.6rem; font-size:0.8rem; color:rgba(150,28,28,0.75); line-height:1.55; }
    ...
    .uf-perm-item { display:flex; align-items:flex-start; gap:0.65rem; padding:0.8rem 0.9rem; border-radius:10px; cursor:pointer; border:1.5px solid rgba(200,140,80,0.12); background:rgba(255,255,255,0.3); transition:all 0.14s; text-align:left; font-family:'DM Sans',sans-serif; }
    ...
    .uf-perm-check { width:17px; height:17px; border-radius:5px; flex-shrink:0; margin-top:0.08rem; border:1.5px solid rgba(200,140,80,0.28); background:rgba(255,255,255,0.5); display:flex; align-items:center; justify-content:center; transition:all 0.14s; }
    ...
    .uf-default-tag { display:inline-block; padding:0.1rem 0.4rem; border-radius:4px; margin-left:0.3rem; font-size:0.61rem; font-weight:600; background:rgba(16,185,129,0.08); border:1px solid rgba(16,185,129,0.18); color:rgba(4,120,87,0.7); }
    .uf-perm-count { margin-top:0.875rem; padding:0.6rem 0.9rem; border-radius:9px; background:rgba(16,185,129,0.05); border:1px solid rgba(16,185,129,0.15); font-size:0.79rem; color:rgba(4,120,87,0.8); }
    .uf-tag-btn { padding:0.3rem 0.7rem; border-radius:7px; font-family:'DM Sans',sans-serif; font-size:0.72rem; font-weight:500; cursor:pointer; transition:all 0.14s; border:1px solid; letter-spacing:-0.01em; }
    ...
    .uf-btn-cancel { flex:1; padding:0.78rem; border-radius:10px; font-family:'DM Sans',sans-serif; font-size:0.84rem; font-weight:500; cursor:pointer; text-align:center; background:rgba(255,255,255,0.45); border:1px solid rgba(200,140,80,0.18); color:rgba(120,60,10,0.7); text-decoration:none; transition:all 0.14s; display:block; }
    .uf-btn-cancel:hover { background:rgba(255,255,255,0.75); }
    .uf-btn-submit { flex:1; padding:0.78rem; border-radius:10px; font-family:'DM Sans',sans-serif; font-size:0.84rem; font-weight:500; cursor:pointer; background:rgba(185,28,28,0.08); border:1px solid rgba(185,28,28,0.22); color:rgba(185,28,28,0.9); transition:all 0.14s; }
    ...
    @media (max-width:700px) { .uf-grid-2{grid-template-columns:1fr} .uf-roles-grid{grid-template-columns:1fr} .uf-perm-grid{grid-template-columns:1fr} .uf-content{padding:1.25rem 1rem 2.5rem} .uf-section{padding:1.25rem} .uf-h1{font-size:1.25rem} .uf-perms-header{flex-direction:column;align-items:flex-start} }
    @media (max-width:400px) { .uf-actions-row{flex-direction:column} }
`;

export default function UsuariosEdit({ usuario, permisos_disponibles, permisos_default }) {
    ...
    const submit                 = (e) => { e.preventDefault(); put(`/usuarios/${usuario.id}`); };
            ...
            <div className="uf-root">
                <div className="uf-header">
                    <div className="uf-header-inner">
                        ...
                        <div>
                            <h1 className="uf-h1">Editar usuario</h1>
                            <p className="uf-sub">{usuario.name} · {usuario.email}</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="uf-content">
                        ...
                        <div className="uf-section">
                            ...
                            <div className="uf-grid-2">
                                <div className="uf-field">
                                    ...
                                    <input className="uf-input" type="text" value={data.name} onChange={e => setData('name', e.target.value)}/>
                                    {errors.name && <p className="uf-error">{errors.name}</p>}
                                </div>
                                <div className="uf-field">
                                    <label className="uf-label">Correo electrónico</label>
                                    <input className="uf-input" type="email" value={data.email} onChange={e => setData('email', e.target.value)}/>
                                    {errors.email && <p className="uf-error">{errors.email}</p>}
                                </div>
                                <div className="uf-field">
                                    <label className="uf-label">Nueva contraseña <span className="uf-label-opt">— opcional</span></label>
                                    <input className="uf-input" type="password" value={data.password} onChange={e => setData('password', e.target.value)} placeholder="Dejar vacío para no cambiar"/>
                                    {errors.password && <p className="uf-error">{errors.password}</p>}
                                </div>
                                <div className="uf-field">
                                    <label className="uf-label">Confirmar contraseña</label>
                                    <input className="uf-input" type="password" value={data.password_confirmation} onChange={e => setData('password_confirmation', e.target.value)} placeholder="Repite la nueva contraseña"/>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="uf-section" style={{animationDelay:'0.05s'}}>
                            ...
                            <div className="uf-roles-grid">
                                <button type="button" className={`uf-role-card ${data.rol === 'admin' ? 'admin-sel' : ''}`}
                                    ...
                                    <p className="uf-role-desc">Acceso total al sistema.</p>
                                </button>
                                <button type="button" className={`uf-role-card ${data.rol === 'empleado' ? 'emp-sel' : ''}`}
                                    ...
                                    <p className="uf-role-desc">Acceso limitado con permisos configurables.</p>
                                </button>
                            </div>
                        </div>
                        ...
                        {!esAdmin && (
                            <div className="uf-section" style={{animationDelay:'0.1s'}}>
                                ...
                                <p className="uf-perms-desc" style={{marginTop:'0.75rem'}}>
                                    Configura qué módulos puede usar este empleado. <strong style={{color:'rgba(4,120,87,0.75)'}}>Predeterminados</strong> restaura los permisos esenciales de un vendedor.
                                </p>
                                ...
                                {data.permisos.length > 0 && (
                                    <div className="uf-perm-count">{data.permisos.length} permiso{data.permisos.length !== 1 ? 's' : ''} activo{data.permisos.length !== 1 ? 's' : ''}</div>
                                )}
                            </div>
                        )}
                        ...
                        {esAdmin && (
                            <div className="uf-info-box">
                                ...
                                <span><strong>Administrador:</strong> acceso completo al sistema. No requiere configuración de permisos.</span>
                            </div>
                        )}
