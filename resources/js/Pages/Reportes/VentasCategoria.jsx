                ...
                return (
                    <div key={d.categoria}>
                        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'0.3rem' }}>
                            <span style={{ fontSize:'0.82rem', fontWeight:'500', color:'#2d1a08', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', maxWidth:'60%' }}>{d.categoria}</span>
                            <span style={{ fontSize:'0.82rem', fontWeight:'700', color:'#2d1a08', flexShrink:0 }}>{typeof val === 'number' && val > 1000 ? fmt(val) : val}</span>
                        </div>
...
function DonaChart({ comparativa, grupoColorMap }) {
    ...
    if (total === 0) return <p style={{ textAlign:'center', fontSize:'0.85rem', color:'rgba(150,80,20,0.45)', padding:'2rem 0' }}>Sin datos</p>;
    ...
    return (
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center' }}>
            <svg width="120" height="120" viewBox="0 0 120 120">
                ...
                <text x={cx} y={cy + 8} textAnchor="middle" fontSize="10" fill="#2d1a08" fontWeight="600">{comparativa.length} grupos</text>
            </svg>
    ...
    return (
        <AppLayout>
            <style>{`
                ...
                .vc-a3{animation:staggerUp .55s cubic-bezier(.16,1,.3,1) .19s both}
                .vc-a4{animation:staggerUp .55s cubic-bezier(.16,1,.3,1) .26s both}
                .vc-a5{animation:staggerUp .55s cubic-bezier(.16,1,.3,1) .33s both}
                ...
                .vc-table-row { transition:background .15s; border-bottom:1px solid rgba(255,255,255,0.3); }
                .vc-table-row:hover { background:rgba(255,255,255,0.12); }
                .vc-table-row:last-child { border-bottom:none; }

                /* Toggle */
                .vc-toggle-btn { padding:0.4rem 0.85rem; font-size:0.78rem; font-weight:500; border:none; cursor:pointer; font-family:'Inter',sans-serif; transition:all .15s; }
                ...
                @media(max-width:900px) { .vc-bars-grid { grid-template-columns:1fr; } }
                @media(max-width:520px) { .vc-kpi-grid { grid-template-columns:1fr 1fr; gap:0.75rem; } }
                ...
                <div className="vc-header">
                    <div style={{ maxWidth:'1280px', margin:'0 auto', padding:'1.5rem' }}>
                        <div style={{ display:'flex', alignItems:'center', gap:'1rem' }}>
                            ...
                            <div>
                                <h1 style={{ fontSize:'clamp(1.2rem,4vw,1.65rem)', fontWeight:'300', color:'#2d1a08', letterSpacing:'-0.03em', lineHeight:1 }}>Ventas por Categoría</h1>
                ...
                <div style={{ maxWidth:'1280px', margin:'0 auto', padding:'1.5rem 1rem 3rem', display:'flex', flexDirection:'column', gap:'1.25rem' }}>
                        ...
                        {[
                            ...
                            <div key={label} className="vc-glass" style={{ padding:'1.3rem' }}>
                                <div style={{ width:'34px', height:'34px', borderRadius:'10px', background:accentBg, border:`1px solid ${accentBorder}`, display:'flex', alignItems:'center', justifyContent:'center', marginBottom:'0.8rem' }}>
                                    <svg width="16" height="16" fill="none" stroke={accent} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.8" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                                </div>
                                <p style={{ fontSize:small ? '0.95rem' : '1.45rem', fontWeight:'600', color:accent, letterSpacing:'-0.02em', lineHeight:1, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{value}</p>
                                ...
                                {comparativa.map((c, i) => {
                                    ...
                                    return (
                                        <div key={c.grupo} style={{ padding:'0.9rem 1rem', background:col.bg, border:`1px solid ${col.border}`, borderRadius:'14px' }}>
                                            ...
                                            <p style={{ fontSize:'1.25rem', fontWeight:'700', color:col.text, margin:'0.2rem 0 0', letterSpacing:'-0.02em' }}>{fmt(c.ingresos)}</p>
                                            <p style={{ fontSize:'0.72rem', color:'rgba(120,60,10,0.55)', marginTop:'0.15rem' }}>{c.unidades} unidades vendidas</p>
                                        </div>
                                    );
                                })}
                                {comparativa.length === 0 && <p style={{ textAlign:'center', fontSize:'0.85rem', color:'rgba(150,80,20,0.4)', padding:'1rem 0' }}>Sin ventas en el período</p>}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="vc-glass vc-a4" style={{ overflow:'hidden' }}>
                        <div style={{ padding:'1.25rem 1.5rem', borderBottom:'1px solid rgba(255,255,255,0.38)' }}>
                            <h2 style={{ fontSize:'0.95rem', fontWeight:'600', color:'#2d1a08', margin:0 }}>Tabla completa por subcategoría</h2>
                            <p style={{ fontSize:'0.76rem', color:'rgba(150,80,20,0.55)', marginTop:'0.2rem' }}>{porCategoria.length} subcategoría{porCategoria.length !== 1 ? 's' : ''} con ventas en el período</p>
                        </div>
                        <div style={{ overflowX:'auto' }}>
                            <table style={{ width:'100%', borderCollapse:'collapse', minWidth:'540px' }}>
                                ...
                                {porCategoria.length === 0 ? (
                                    <tr><td colSpan="6" style={{ padding:'3rem', textAlign:'center', fontSize:'0.88rem', color:'rgba(150,80,20,0.4)' }}>Sin ventas en el período seleccionado</td></tr>
                                    ...
                                    return (
                                        <tr key={cat.categoria} className="vc-table-row">
                                            ...
                                            <td style={{ padding:'0.85rem 1.25rem' }}>
                                                <span style={{ fontSize:'0.72rem', fontWeight:'600', padding:'0.2rem 0.65rem', borderRadius:'20px', background:col.bg, border:`1px solid ${col.border}`, color:col.text }}>{cat.grupo}</span>
                                            </td>
